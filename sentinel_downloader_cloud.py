...
with st.sidebar:
    ...
    if "GIF" in formato_descarga or "Todos" == formato_descarga:
        ...
        st.info("🎯 El GIF se centrará en la fecha elegida (mitad antes, mitad después).")
    ...
    if st.button(f"🔍 Buscar Imágenes"):
        with st.spinner("Consultando catálogo STAC..."):
            try:
                ...
                # Buscamos por separado para asegurar el "centro"
                ...
                st.success(f"Búsqueda finalizada. Candidatas antes: {len(st.session_state['scenes_before'])}, Candidatas después: {len(st.session_state['scenes_after'])}")
                st.rerun()
    ...
    if 'scenes_before' in st.session_state and 'scenes_after' in st.session_state:
        ...
        if "GIF" not in formato_descarga:
            ...
            if st.button("🎬 Generar GIF Multi-Temporal Centrado"):
                with st.status("Buscando equilibrio temporal...") as status:
                    ...
                    # 1. Buscamos primero la mitad "después" (hacia el futuro desde la fecha de ref)
                    found_after = process_pool(pool_after, target_half + (target_total % 2), "futuro")
                    
                    # 2. Buscamos la mitad "antes" (hacia el pasado desde la fecha de ref)
                    remaining = target_total - found_after
                    found_before = process_pool(pool_before, remaining, "pasado")
                    
                    # 3. Si aún falta cupo porque un lado se agotó, intentamos completar con el otro
                    ...
                    if frames_list:
                        # Ordenamos el resultado final cronológicamente para que el GIF fluya bien
                        ...
                        st.error("No se encontraron imágenes válidas para el GIF.")

st.markdown("---")
st.caption("Equilibrio temporal activado: Mitad antes y mitad después de la fecha de referencia.")