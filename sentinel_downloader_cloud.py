...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("3. Importar AOI (Opcional)")
    uploaded_file = st.file_uploader("Subir GeoJSON, KML o Shapefile (.zip)", type=['geojson', 'kml', 'zip'])
...
if uploaded_file is not None:
    try:
        if uploaded_file.name.endswith('.zip'):
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if shp_files:
                    ...
                    st.error("No se encontró un archivo .shp dentro del ZIP.")
                    gdf = None
        else:
        ...
        if gdf is not None:
            ...
            bbox_file = list(gdf.total_bounds)
            gdf_display = gdf
            st.sidebar.success(f"AOI cargada: {uploaded_file.name}")
    except Exception as e:
        st.sidebar.error(f"Error procesando archivo: {e}")

# --- MAPA ---
st.subheader("1. Área de Interés (AOI)")
map_center = [-35.444, -60.884]
if bbox_file:
    map_center = [(bbox_file[1] + bbox_file[3]) / 2, (bbox_file[0] + bbox_file[2]) / 2]

m = folium.Map(location=map_center, zoom_start=13 if not bbox_file else 14)
LocateControl(auto_start=not bbox_file).add_to(m)

if gdf_display is not None:
    folium.GeoJson(gdf_display, name="AOI Importada", style_function=lambda x: {'color': 'red', 'fillOpacity': 0.1}).add_to(m)
...
if not bbox:
    st.warning("Dibuja un rectángulo o sube un archivo para comenzar.")
else:
    st.success(f"AOI activa: {bbox}")
    ...
    if 'scenes_list' in st.session_state:
        ...
        if "GIF" not in formato_descarga:
                ...
                try:
                    with st.status("Procesando..."):
                        ...
                        if "GeoTIFF" in formato_descarga or "Todos" == formato_descarga:
                            with tempfile.NamedTemporaryFile(suffix='.tif', delete=False) as tmp:
                                ...
                                with open(tmp.name, 'rb') as f: 
                                    st.download_button(f"📥 {fname}.tif", f.read(), f"{fname}.tif", key=f"dl_tif_{fname}")
                        ...
                        if "JPG" in formato_descarga or "Todos" == formato_descarga:
                            ...
                            st.download_button(f"📷 {fname}.jpg", buf.getvalue(), f"{fname}.jpg", key=f"dl_jpg_{fname}")
                except Exception as e: st.error(f"Error descarga: {e}")
        else:
            ...
            if st.button("🎬 Generar GIF Multi-Temporal"):
                ...
                except Exception as e: st.error(f"Error GIF: {e}")

st.markdown("---")
st.caption("Satellite Downloader | AOI Import | Landsat 8/9 & Sentinel-2 Support | GIF Generator 40 Frames")