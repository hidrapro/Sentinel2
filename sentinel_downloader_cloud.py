...
SAT_CONFIG = {
    ...
    "Landsat 8/9": {
        ...
        "res": 30,
        "tile_key": "landsat:wrs_path", # Simplificado para nombre de archivo
        "cloud_key": "eo:cloud_cover",
        "scale": 0.0000275,
        "offset": -0.2
    }
...
# --- PROCESAMIENTO ---
def normalize_image_robust(img_array, percentile_low=2, percentile_high=98, is_landsat=False):
    # Aplicar escala Landsat si corresponde ANTES de normalizar
    if is_landsat:
        img_array = img_array * 0.0000275 - 0.2
    ...
    img_stretched = np.clip((img_work - p_low) / (max(1e-5, p_high - p_low)), 0, 1)
    ...
    if 'scenes_list' in st.session_state:
        ...
        if "GIF" not in formato_descarga:
            ...
            if st.button("🖼️ Vista Previa"):
                ...
                img = normalize_image_robust(np.moveaxis(data.values, 0, -1), percentil_bajo, percentil_alto, "Landsat" in sat_choice)
            ...
            if st.button(f"🚀 Descargar {sat_choice} HD"):
                with st.status("Procesando..."):
                    ...
                    if "JPG" in formato_descarga or "Todos" == formato_descarga:
                        img_8bit = normalize_image_robust(np.moveaxis(data.values, 0, -1), percentil_bajo, percentil_alto, "Landsat" in sat_choice)
                        buf = io.BytesIO()
            ...
            if st.button("🎬 Generar GIF Multi-Temporal"):
                with st.status("Procesando frames...") as status:
                    ...
                    for i, s in enumerate(subset):
                        ...
                        # Discriminador de bordes
                        if np.mean(np.any(np.isnan(img_raw) | (img_raw <= 0), axis=-1)) > 0.10:
                            skipped += 1; continue
                        
                        img_8bit = normalize_image_robust(img_raw, percentil_bajo, percentil_alto, "Landsat" in sat_choice)
...
st.markdown("---")
st.caption("Soporte Landsat 8/9 & Sentinel-2 | Corrección de escala Landsat C2 L2 | Discriminador de bordes")