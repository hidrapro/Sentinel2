...
def normalize_image_robust(img_array, percentile_low=2, percentile_high=98, scale=1.0, offset=0.0):
    ...
    if len(img_work.shape) == 3:
        ...
        for i in range(img_work.shape[2]):
            channel = img_work[:, :, i]
            valid_mask = (channel > 0) & (~np.isnan(channel))
        ...
        p_low = np.percentile(img_work[valid_mask], percentile_low) if np.any(valid_mask) else 0
        p_high = np.percentile(img_work[valid_mask], percentile_high) if np.any(valid_mask) else 1
        denom = max(1e-5, p_high - p_low)
