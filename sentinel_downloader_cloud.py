...
def normalize_image_robust(img_array, percentile_low=2, percentile_high=98, scale=1.0, offset=0.0):
    ...
    # Si tenemos menos de 3 canales después de moveaxis (lo manejamos en la lógica principal)
    # pero aquí devolvemos la matriz estirada
...
def prepare_for_streamlit(img_array):
    ...
    if img_array.ndim == 2:
        return img_array # Grayscale 2D es aceptado
        ...
        if "GIF" not in formato_descarga:
            ...
            if not scene_opts:
                ...
                if st.button("🖼️ Vista Previa"):
                    # Usamos isel(time=0) en lugar de squeeze() para evitar colapsar dimensiones de banda
