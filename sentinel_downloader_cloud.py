...
# Sentinel-2: Reflectancia 0-10000 (aprox)
# Landsat C2 L2: DN 0-65535 que requieren escala y offset
SAT_CONFIG = {
    "Sentinel-2": {
...
with st.sidebar:
    ...
    if "GIF" in formato_descarga or "Todos" == formato_descarga:
        ...
        gif_max_images = st.slider("Máx imágenes", 3, 30, 12)
...
def normalize_image_robust(img_array, percentile_low=2, percentile_high=98, scale=1.0, offset=0.0):
    ...
    1. Identifica NaNs y valores nulos brutos (nodata).
    2. Aplica escala y offset del satélite.
    3. Calcula percentiles solo sobre píxeles con datos válidos.
    """
    img_work = np.copy(img_array).astype(np.float32)
    
    # Máscara de datos reales (basada en valores brutos antes del offset)
    # Landsat C2 usa 0 para nodata.
    valid_mask = (img_work > 0) & (~np.isnan(img_work))
    
    # Aplicar escala y offset (Landsat: * 0.0000275 - 0.2)
    img_work = img_work * scale + offset
    
    # Clip de seguridad (la reflectancia no puede ser negativa)
    img_work = np.clip(img_work, 0, 1)
    
    if np.sum(valid_mask) < 100:
        return (img_work * 255).astype(np.uint8)
    
    # Cálculo de percentiles sobre datos válidos escalados
    ...
    # Estiramiento de contraste (Contrast Stretching)
    denom = max(1e-5, p_high - p_low)
    img_stretched = np.clip((img_work - p_low) / denom, 0, 1)
    ...
    if st.button(f"🔍 Buscar en {sat_choice}"):
        with st.spinner("Conectando con Planetary Computer..."):
            try:
                ...
                s_before = catalog.search(**common_args, datetime=f"2013-01-01/{fecha_referencia.isoformat()}", max_items=20, sortby=[{"field":"properties.datetime","direction":"desc"}])
                s_after = catalog.search(**common_args, datetime=f"{fecha_referencia.isoformat()}/{datetime.now().isoformat()}", max_items=20, sortby=[{"field":"properties.datetime","direction":"asc"}])
            ...
            if st.button(f"🚀 Descargar {sat_choice} HD"):
                with st.status("Procesando..."):
                    ...
                    if "JPG" in formato_descarga or "Todos" == formato_descarga:
                        ...
                        st.download_button(f"📷 {fname}.jpg", buf.getvalue(), f"{fname_base}.jpg" if 'fname_base' in locals() else f"{fname}.jpg")
        else:
...
st.markdown("---")
st.caption("Soporte Landsat 8/9 & Sentinel-2 | Corrección de escala Landsat C2 L2 | Discriminador de bordes corregido")