...
with st.sidebar:
    ...
    if formato_descarga in ["JPG (para visualizar)", "Ambos"]:
        st.markdown("---")
        st.subheader("Ajustes JPG")
        percentil_bajo = st.slider("Corte inferior (%)", 0, 10, 2, 
                                    help="Elimina valores muy oscuros (sombras)")
        percentil_alto = st.slider("Corte superior (%)", 90, 100, 98, 
                                    help="Elimina valores muy brillantes (saturación)")
...
def normalize_image_percentile(img_array, percentile_low=2, percentile_high=98):
    """
    Normaliza la imagen usando percentiles para mejor contraste.
    
    Args:
        img_array: array (height, width, 3)
        percentile_low: percentil inferior (elimina valores oscuros extremos)
        percentile_high: percentil superior (elimina valores brillantes extremos)
    
    Returns:
        array normalizado (0-255) tipo uint8
    """
    ...
    for i in range(img_array.shape[2]):  # Para cada banda
        ...
        # Eliminar valores extremos usando percentiles
        ...
        with st.expander("🖼️ Vista Previa Rápida", expanded=True):
            if st.button("Generar Preview"):
                try:
                    ...
                    # Aplicar la misma normalización que se usará en el JPG
                    img_preview = normalize_image_percentile(img, 2, 98)
                    st.image(img_preview, use_container_width=True, caption="Preview con normalización mejorada")
                except Exception as e:
        ...
        if st.button("🚀 Descargar Imagen HD (10m)"):
            try:
                with st.status("Procesando descarga HD...", expanded=True) as status:
                    ...
                    # --- OPCIÓN 2: JPG CON NORMALIZACIÓN MEJORADA ---
                    if formato_descarga in ["JPG (para visualizar)", "Ambos"]:
                        # Usar valores de percentiles del sidebar
                        p_low = percentil_bajo if 'percentil_bajo' in locals() else 2
                        p_high = percentil_alto if 'percentil_alto' in locals() else 98
                        
                        # Normalizar con percentiles
                        img_8bit = normalize_image_percentile(img_array, p_low, p_high)
                    ...
                    st.info("💡 **Tip:** El JPG usa normalización por percentiles para mejor contraste. Ajusta los sliders en el sidebar si quieres más/menos contraste.")
...
st.markdown("---")
st.caption(f"9 de Julio, Argentina | Búsqueda inteligente +/- 10 imágenes | Normalización adaptativa para JPG")
