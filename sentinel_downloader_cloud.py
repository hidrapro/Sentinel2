...
def check_data_quality(item, conf, bbox, epsg_code, threshold=0.30):
    """Verifica si una escena tiene menos del threshold% de datos inválidos."""
    try:
        # Cargar solo la primera banda a baja resolución para chequeo rápido
        data_check = stackstac.stack(
            item, 
            assets=[conf["assets"][0]], 
            bounds_latlon=bbox, 
            epsg=epsg_code, 
            resolution=conf["res"]*4
        ).squeeze().compute()
        
        # Calcular porcentaje de píxeles inválidos
        invalid_mask = np.isnan(data_check.values) | (data_check.values <= 0)
        invalid_percentage = np.mean(invalid_mask)
        
        return invalid_percentage < threshold
    except:
        return False
...
# Guardar y recuperar la posición del mapa
if 'map_center' not in st.session_state:
    st.session_state['map_center'] = [-35.444, -60.884]
    st.session_state['map_zoom'] = 13

# Actualizar posición si el usuario movió el mapa
if 'last_map_data' in st.session_state and st.session_state['last_map_data']:
    last_data = st.session_state['last_map_data']
    if last_data.get('center'):
        st.session_state['map_center'] = [last_data['center']['lat'], last_data['center']['lng']]
    if last_data.get('zoom'):
        st.session_state['map_zoom'] = last_data['zoom']
...
map_data = st_folium(m, width=1200, height=400, key=f"main_map_{map_style}")

# Guardar datos del mapa actual
...
if bbox:
    ...
    if 'scenes_before' in st.session_state:
        ...
        # Filtrar escenas con mala calidad de datos (solo si aún no se ha filtrado)
        if 'filtered_scenes' not in st.session_state or st.session_state.get('last_bbox') != bbox:
            if all_scenes and epsg_code:
                with st.spinner(f"Filtrando escenas con datos insuficientes..."):
                    filtered = []
                    for scene in all_scenes:
                        if check_data_quality(scene, conf, bbox, epsg_code):
                            filtered.append(scene)
                    st.session_state['filtered_scenes'] = filtered
                    st.session_state['last_bbox'] = bbox
                    original_count = len(all_scenes)
                    filtered_count = len(filtered)
                    if filtered_count < original_count:
                        st.info(f"🔍 Filtradas {original_count - filtered_count} escenas con >30% de datos inválidos. Escenas válidas: {filtered_count}")
            else:
                st.session_state['filtered_scenes'] = all_scenes
        
        all_scenes = st.session_state.get('filtered_scenes', all_scenes)
        
        if not all_scenes:
            st.warning("No hay escenas disponibles con suficientes datos válidos.")
        else:
