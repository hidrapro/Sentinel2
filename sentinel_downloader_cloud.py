...
# Nota: Para Landsat 7, las bandas 4, 5 y 3 en PC se acceden mediante alias comunes
SAT_CONFIG = {
    "Sentinel-2": {
    ...
    "Landsat 7": {
        "collection": "landsat-c2-l2",
        "platform": ["landsat-7"],
        "assets": ["nir08", "swir16", "red"], # Corresponde a B4, B5, B3 en L7
        "res": 30,
        "tile_key": "landsat:wrs_path",
        "cloud_key": "eo:cloud_cover",
        "scale": 0.0000275,
...
with st.sidebar:
    ...
    with col_a:
        anio = st.number_input("Año", min_value=conf["min_year"], max_value=datetime.now().year, value=datetime.now().year)
    ...
    if st.button(f"🔍 Buscar Imágenes"):
        with st.spinner("Consultando catálogo STAC..."):
            try:
                ...
                # Query base con filtro de nubes
                query_params = {conf["cloud_key"]: {"lt": max_cloud}}
                
                # Si el satélite tiene plataformas específicas (L7 o L8/9)
                if conf["platform"]:
        ...
        if "GIF" not in formato_descarga:
            ...
            if not scene_opts:
                ...
                if st.button("🖼️ Vista Previa"):
                    # Forzamos una resolución más baja para la previa rápida
...
st.markdown("---")
st.caption("Nota: Landsat 7 incluye datos desde 1999. Recuerda que las imágenes posteriores a 2003 pueden presentar las franjas negras características del fallo del sensor SLC.")