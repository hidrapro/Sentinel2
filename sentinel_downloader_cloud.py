...
# Hemos añadido la banda 'green' a todos para soportar NDWI y otros análisis.
SAT_CONFIG = {
    "Sentinel-2": {
        "collection": "sentinel-2-l2a",
        "platform": None,
        "assets": ["B08", "B04", "B03", "B11"], # NIR, RED, GREEN, SWIR
        "res": 10,
...
with st.sidebar:
    ...
    producto = st.selectbox(
        ...
        help="Color: Composición de bandas. NDVI: Índice de vegetación (-1 a 1). NDWI: Índice de agua."
    )
...
def calculate_index(data, prod_type, sat_type):
    """Calcula NDVI o NDWI basado en los nombres de bandas de STAC."""
    # Mapeo genérico basado en assets comunes
    nir = data.sel(band=conf["assets"][0]) # Usualmente nir08 o B08
    red = data.sel(band=conf["assets"][1]) # Usualmente red o B04
    green = data.sel(band=conf["assets"][2]) # Usualmente green o B03
    
    if prod_type == "NDVI (Vegetación)":
        return (nir - red) / (nir + red + 1e-10)
    elif prod_type == "NDWI (Agua)":
        return (green - nir) / (green + nir + 1e-10)
    return data # Caso Color no entra aquí
            ...
            if formato_descarga != "GIF Animado":
                ...
                with col1:
                    if st.button("🖼️ Vista Previa"):
                        # Si el producto es un índice, seleccionamos solo las bandas necesarias para el cálculo
                        # Para Color usamos las 3 primeras (NIR, SWIR/RED, RED/GREEN) según config
                    ...
                    if st.button("🚀 Descargar HD"):
                        with st.status("Procesando datos HD..."):
                            ...
                            if "JPG" in formato_descarga or "Todos" == formato_descarga:
                                ...
                                # Si es un índice, np.moveaxis no es necesario (es 2D) o es diferente
