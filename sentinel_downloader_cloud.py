...
def normalize_image_robust(img_array, percentile_low=2, percentile_high=98):
    ...
    Esta versión calcula los percentiles sobre TODAS las bandas juntas,
    no independientemente, lo que evita que una banda se sature y cause tintes verdes.
    """
    # Crear máscara para valores válidos (eliminar 0s y NaNs)
    valid_mask = (img_array > 0) & (~np.isnan(img_array))
    
    # Si hay muy pocos valores válidos, usar normalización básica
    ...
    # Calcular percentiles GLOBALES (sobre todas las bandas juntas)
    ...
    # Aplicar stretch lineal a TODAS las bandas con los mismos valores
...
def normalize_image_per_band(img_array, percentile_low=2, percentile_high=98):
    """
    Normalización independiente por banda (puede causar tintes de color).
    Usar solo si el método robusto falla.
    """
    img_normalized = np.zeros_like(img_array, dtype=np.float32)
    
    for i in range(img_array.shape[2]):  # Para cada banda
        band = img_array[:, :, i]
        
        # Crear máscara de valores válidos
        valid_mask = (band > 0) & (~np.isnan(band))
        
        if np.sum(valid_mask) < 50:  # Si hay muy pocos valores válidos
            img_normalized[:, :, i] = band / 3000  # Normalización simple
            continue
        
        valid_values = band[valid_mask]
        
        # Calcular percentiles solo en valores válidos
        p_low = np.percentile(valid_values, percentile_low)
        p_high = np.percentile(valid_values, percentile_high)
...
def add_text_to_image(img_pil, text, position='bottom'):
    """
    Agrega texto (fecha) a una imagen PIL.
    """
    draw = ImageDraw.Draw(img_pil)
    
    # Calcular tamaño de fuente adaptativo
    img_width = img_pil.width
    font_size = max(20, img_width // 40)
    ...
    if position == 'bottom':
        ...
        y = img_pil.height - text_height - padding - 10
    else:  # top
        x = (img_width - text_width) // 2
        y = padding
            ...
            if st.button("🚀 Descargar Imagen HD (10m)"):
                try:
                    with st.status("Procesando descarga HD...", expanded=True) as status:
                        ...
                        # --- OPCIÓN 2: JPG CON NORMALIZACIÓN ROBUSTA ---
                        if formato_descarga in ["JPG (para visualizar)", "Todos"]:
                            # Usar normalización robusta (global, no por banda)
                        ...
                        st.success("✅ Usando normalización robusta (percentiles globales)")
            ...
            with st.expander("📅 Fechas incluidas en el GIF", expanded=True):
                ...
                for idx, scene in enumerate(available_scenes):
                    with cols[idx % 4]:
                        ...
                        st.caption(f"Nubes: {scene.properties['eo:cloud_cover']:.1f}%")
            
            if st.button("🎬 Generar GIF Animado (puede tomar varios minutos)"):
                try:
                    with st.status(f"Procesando {len(available_scenes)} imágenes...", expanded=True) as status:
                        frames = []
                        
                        for idx, scene in enumerate(available_scenes):
                            status.update(label=f"Procesando imagen {idx+1}/{len(available_scenes)}: {scene.datetime.strftime('%d/%m/%Y')}...", state="running")
                        ...
                        # Preview del GIF
                        st.image(gif_bytes, caption=f"Preview: {len(frames)} frames, {duration}ms por frame")
                        ...
                        st.success(f"✅ GIF creado: {len(frames)} frames, {len(gif_bytes)//1024} KB (normalización robusta)")
...
st.markdown("---")
st.caption(f"9 de Julio, Argentina | Normalización robusta anti-saturación | GIF animado de series temporales")
