...
with st.sidebar:
    ...
    if formato_descarga in ["GIF Animado (serie temporal)", "Todos"]:
        ...
        # Se aumentó el límite máximo de imágenes de 15 a 30
        gif_max_images = st.slider("Máx. imágenes en GIF", 3, 30, 8)
...
def normalize_image_robust(img_array, percentile_low=2, percentile_high=98):
    # Asegurarnos de trabajar con una copia para no alterar el original y manejar NaNs
    img_work = np.nan_to_num(img_array, nan=0.0)
    valid_mask = (img_work > 0)
    
    if np.sum(valid_mask) < 100:
        return (np.clip(img_work / 3000, 0, 1) * 255).astype(np.uint8)
    
    valid_values = img_work[valid_mask]
    ...
    img_stretched = np.clip((img_work - p_low) / (p_high - p_low), 0, 1)
    ...
    if st.button("🔍 Buscar imágenes"):
        with st.spinner("Consultando catálogo..."):
            try:
                ...
                # Se aumentó max_items de 10 a 20 por búsqueda para permitir hasta 40 resultados totales (filtrables a 30)
                search_before = catalog.search(collections=["sentinel-2-l2a"], bbox=bbox, datetime=f"2015-06-01/{fecha_referencia.isoformat()}", max_items=20, query={"eo:cloud_cover": {"lt": max_cloud}}, sortby=[{"field": "properties.datetime", "direction": "desc"}])
                search_after = catalog.search(collections=["sentinel-2-l2a"], bbox=bbox, datetime=f"{fecha_referencia.isoformat()}/{datetime.now().isoformat()}", max_items=20, query={"eo:cloud_cover": {"lt": max_cloud}}, sortby=[{"field": "properties.datetime", "direction": "asc"}])
            ...
            if st.button("🚀 Descargar Imagen HD (10m)"):
                try:
                    with st.status("Procesando HD...", expanded=True):
                        ...
                        fname_base = f"T{tile}_{date_str}"
                        
                        if formato_descarga in ["GeoTIFF (para GIS)", "Todos"]:
                            with tempfile.NamedTemporaryFile(delete=False, suffix='.tif') as tmp:
                                data.rio.to_raster(tmp.name)
                                with open(tmp.name, 'rb') as f:
                                    st.download_button(f"📥 Descargar {fname_base}.tif", f.read(), f"{fname_base}.tif")
                        ...
                        if formato_descarga in ["JPG (para visualizar)", "Todos"]:
                            ...
                            st.download_button(f"📷 Descargar {fname_base}.jpg", buf.getvalue(), f"{fname_base}.jpg")
                except Exception as e:
                    st.error(f"Error: {e}")
        else:
            ...
            if st.button("🎬 Generar GIF"):
                try:
                    with st.status("Generando frames...", expanded=True) as status:
                        frames, skipped = [], 0
                        max_f = scenes[:gif_max_images]
                        for idx, scene in enumerate(max_f):
                            status.update(label=f"Analizando imagen {idx+1}/{len(max_f)}...")
                            ...
                            # Un píxel es nodata si es NaN o si todas las bandas son 0
                            is_nodata = np.any(np.isnan(img_array), axis=-1) | np.all(img_array <= 0, axis=-1)
                            nodata_ratio = np.mean(is_nodata)
                            
                            # Filtro estricto: si más del 5% es negro/vacio, fuera.
                        ...
                        if frames:
                            ...
                            st.image(buf.getvalue(), caption=f"Animación final: {len(frames)} imágenes válidas")
                            st.download_button("🎬 Descargar GIF", buf.getvalue(), "serie_temporal.gif")
                            if skipped > 0: 
                                st.warning(f"Se omitieron {skipped} imágenes por tener áreas negras o estar fuera del área de cobertura (borde de captura).")
                        else:
                            st.error("No hay suficientes imágenes completas en este período para generar un GIF. Intenta ajustar el área o los filtros.")
                except Exception as e:
                    st.error(f"Error: {e}")

st.markdown("---")
st.caption(f"Sentinel Downloader | Geolocalización habilitada | Discriminador de bordes corregido (NaN + Black)")