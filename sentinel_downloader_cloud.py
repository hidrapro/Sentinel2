...
with st.sidebar:
    ...
    if "GIF" in formato_descarga or "Todos" == formato_descarga:
        ...
        gif_max_images = st.slider("Cantidad de imágenes objetivo", 3, 30, 10)
        st.info("☁️ El sistema buscará imágenes válidas hasta completar la cantidad pedida.")
        local_cloud_filter = st.slider("Filtro Nubes Local (Umbral Brillo)", 150, 255, 215, help="Si el brillo promedio supera esto, se descarta y se busca la siguiente fecha.")
...
if bbox:
    if st.button(f"🔍 Buscar Imágenes"):
        with st.spinner("Consultando catálogo STAC..."):
            try:
                ...
                # Buscamos un pool grande de imágenes para tener de donde elegir si hay nubes
                common_args = {"collections": [conf["collection"]], "bbox": bbox, "query": {conf["cloud_key"]: {"lt": max_cloud}}}
                
                s_before = catalog.search(**common_args, datetime=f"2013-01-01/{fecha_referencia.isoformat()}", max_items=50, sortby=[{"field":"properties.datetime","direction":"desc"}])
                s_after = catalog.search(**common_args, datetime=f"{fecha_referencia.isoformat()}/{datetime.now().isoformat()}", max_items=50, sortby=[{"field":"properties.datetime","direction":"asc"}])
                
                scenes = list(s_before.items()) + list(s_after.items())
                scenes.sort(key=lambda x: x.datetime)
                if scenes:
                    st.session_state['scenes_list'] = scenes
                    st.success(f"Encontradas {len(scenes)} imágenes candidatas en el catálogo.")
    ...
    if 'scenes_list' in st.session_state:
        ...
        if "GIF" not in formato_descarga:
            # Lógica de descarga individual (sin cambios significativos aquí)
            ...
            # --- NUEVA LÓGICA DE GIF EXHAUSTIVA ---
            if st.button("🎬 Generar GIF Multi-Temporal (Búsqueda Exhaustiva)"):
                with st.status("Buscando imágenes válidas...") as status:
                    frames = []
                    skipped_nodata = 0
                    skipped_clouds = 0
                    
                    # Intentamos procesar las escenas de forma distribuida
                    # pero si una falla, probamos con la siguiente inmediatamente disponible
                    ...
                    for i, s in enumerate(scenes):
                        ...
                        status.update(label=f"Evaluando {current_date_str} (Encontrados: {len(frames)}/{gif_max_images})...")
                        
                        try:
                            # Carga rápida (baja res) para validación
                            ...
                            # 1. Filtro de Datos faltantes
                            if np.mean(np.any(np.isnan(img_raw) | (img_raw <= 0), axis=-1)) > 0.15:
                                skipped_nodata += 1
                                continue
                            
                            # 2. Análisis de Brillo (Nubes locales)
                            ...
                            # Si pasa los filtros, lo agregamos
                            frames.append(add_text_to_image(Image.fromarray(img_8bit), current_date_str))
                            
                        except Exception as e:
                            st.write(f"Error procesando {current_date_str}: {e}")
                            continue
                    
                    if frames:
                        buf = io.BytesIO()
                        frames[0].save(buf, format='GIF', save_all=True, append_images=frames[1:], duration=gif_duration, loop=0)
                        st.image(buf.getvalue(), caption=f"Serie temporal con {len(frames)} imágenes válidas.")
                        st.download_button("🎬 Descargar GIF", buf.getvalue(), "serie_temporal_completada.gif")
                        
                        st.write(f"✅ Se completaron {len(frames)} frames.")
                        if skipped_clouds > 0 or skipped_nodata > 0:
                            st.info(f"ℹ️ Se descartaron {skipped_clouds} por nubes y {skipped_nodata} por falta de datos, pero se usaron otras fechas para compensar.")
                    else:
                        st.error("No se encontró ni una sola imagen que pase los filtros de calidad en todo el rango buscado.")

st.markdown("---")
st.caption("Búsqueda iterativa activada: el sistema no se detiene hasta completar el cupo o agotar las fechas.")