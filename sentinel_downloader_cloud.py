...
if bbox:
    ...
    if 'scenes_before' in st.session_state:
        # Aplicamos el filtro de exclusión manual
        ...
        if not all_scenes:
            ...
            if "GIF" in formato_descarga or "Todos" == formato_descarga:
                ...
                if st.button("🎬 Generar Animación GIF Multi-Temporal"):
                    frames_list = []
                    # Priorizar imágenes cerca de la fecha y con pocas nubes
                    ...
                    with st.status("Generando frames del GIF...") as status:
                        ...
                        for s in pool:
                            ...
                            try:
                                ...
                                status.update(label=f"Analizando frame {processed_count + 1}: {date_str}...")
                                
                                # Carga rápida para verificar integridad
                                ...
                                # FILTRO DE DATOS FALTANTES (Negros/NaNs)
                                # Consideramos 'vacio' si más del 20% son ceros o NaNs
                                nodata_mask = np.any(np.isnan(img_np) | (img_np <= 0), axis=-1)
                                if np.mean(nodata_mask) > 0.20:
                                    status.update(label=f"Omitiendo {date_str} (demasiado negro/nulo)")
                                    continue
                        ...
                        if frames_list:
                            ...
                            st.image(buf.getvalue(), caption=f"Serie de {len(images_only)} imágenes filtradas.")
                            st.download_button("📥 Descargar GIF", buf.getvalue(), "serie_temporal_filtrada.gif")
                        else:
                            st.error("No se encontraron imágenes con suficientes datos útiles para el GIF.")
