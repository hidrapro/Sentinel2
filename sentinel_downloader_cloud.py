    ...
    if 'scenes_before' in st.session_state:
        ...
        if not all_scenes:
            ...
            if st.button("🖼️ Vista Previa"):
                # Preview: .compute() sobre el DataArray, no sobre .values
                ...
                st.image(img, use_container_width=True, caption=f"Previa balanceada")
            ...
            if st.button("🚀 Descargar HD"):
                with st.status("Procesando datos HD..."):
                    ...
                    if "JPG" in formato_descarga or "Todos" == formato_descarga:
                        # CORRECCIÓN AQUÍ: compute() se llama sobre el DataArray de Xarray
...
st.markdown("---")
st.caption("Nota técnica: Landsat 1-3 usa el sensor MSS que carece de banda azul. La composición NIR-R-G es un Falso Color Vegetal.")
