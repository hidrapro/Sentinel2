    ...
    if img.ndim == 3:
        ...
        for i in range(img.shape[2]):
            ...
            if valid.size > 0:
                ...
                out[:, :, i] = np.clip((band - vmin) / (vmax - vmin) * 255, 0, 255).astype(np.uint8)
        return out
    else:
...
with st.sidebar:
    ...
    if "GIF" in formato_descarga or "Todos" == formato_descarga:
        st.markdown("---")
        st.header("5. GIF Animado")
        gif_duration = st.slider("Duración por frame (ms)", 100, 2000, 500, 100)
        gif_max_images = st.slider("Máximo de frames", 3, 20, 10)
                ...
                if st.button("🎬 Generar Animación GIF"):
                    ...
                    with st.status("Generando frames...") as status:
                        ...
                        if frames_list:
                            ...
                            st.image(buf.getvalue(), caption=f"Serie Temporal RGB - {len(images_only)} frames.")
                            st.download_button("📥 Descargar GIF", buf.getvalue(), "serie_temporal_rgb.gif")
